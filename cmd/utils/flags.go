...
var (
	...
	PeriConfigFlag = DirectoryFlag{
		Name:  "peri",
		Usage: "path to config file of peer selection policy",
		Value: DirectoryString(""),
	}
	LoggyPathFlag = DirectoryFlag{
		Name:  "loggypath",
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	// PERI_AND_LATENCY_RECORDER_CODE_PIECE
	lcfg, err := loggy.NewLoggyConfig(ctx.GlobalString(LoggyPathFlag.Name))
	if err != nil {
		panic(err)
	}
	loggy.Config = lcfg

	pcfg, err := ethconfig.NewPeriConfig(ctx.GlobalString(PeriConfigFlag.Name))
	if err != nil {
		panic(err)
	}
	cfg.PeriConfig = pcfg
